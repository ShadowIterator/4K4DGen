...
def save_data(source_path, pano_img, distances, R, fx, fy, cx, cy, pers_imgs, pts, pers_coords, colors=None):
    ...
    # np.save(os.path.join(source_path, 'pers_sample_coords.npy'), pers_coords.detach().cpu().numpy())
    ...
    if colors is None:
        colors = pano_img.reshape(-1, 3).cpu().numpy()
    pcd = trimesh.PointCloud(pts, colors)
...
def save_data_k(source_path, ind, frames_torch, distances, R, fx, fy, cx, cy, pers_imgs, pts, pers_coords, colors=None):
    pers_coords_path = os.path.join(source_path, 'pers_coord.npy')
    if not os.path.exists(pers_coords_path):
        np.save(pers_coords_path, pers_coords.detach().cpu().numpy())
    save_path = os.path.join(source_path, '''{:02d}'''.format(ind))
    os.makedirs(save_path, exist_ok=True)
    img = frames_torch
    # img = cv2.resize(img.cpu().numpy(), (2048, 1024), cv2.INTER_AREA)
    img = img.cpu().numpy()
    img = torch.from_numpy(img).cuda()
    save_data(save_path, img, distances, R, fx, fy, cx, cy, pers_imgs[:,ind,...], pts, pers_coords, colors)




def save_data_noSTA(source_path, pano_img, distances, R, fx, fy, cx, cy, pers_imgs, pts, pers_coords, colors=None):
    image_folder = os.path.join(source_path, 'images')
    #depth_folder = os.path.join(source_path, 'depths')
    pose_folder = os.path.join(source_path, 'sparse/0')
