...
class OmnidataPredictor(GeoPredictor):
    def __init__(self):
        ...
        self.model = DepthAnything.from_pretrained('LiheYoung/depth_anything_{}14'.format('vits')).cpu().eval()
    ...
    def predict_depth(self, img, **kwargs):
            self.model.to(torch.device('cuda'))
            img_tensor = self.trans_totensor(img)
            img_tensor = F.interpolate(img_tensor, size=(504, 504), mode='bicubic')
            # output = F.interpolate(output[:, None], size=(512, 512), mode='bicubic')
            # output = self.model(img_tensor).clip(0., 1.) * -1 + 1
            output = self.model(img_tensor)
            output = ((output - output.min()) / (output.max() - output.min())).clip(0., 1.) * -1 + 1
            self.model.to(torch.device('cpu'))
            output = F.interpolate(output.unsqueeze(1), size=(512, 512), mode='bicubic').squeeze(1)
            output = output.clip(0., 1.)
            return output[:, None]