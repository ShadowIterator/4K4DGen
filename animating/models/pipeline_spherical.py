...
from .SphericalLatentField import SpherelatentField
...
def split_latent(latents, mask, image_latents, gen_res):
    ...
    # pers_img_latents = rearrange(pers_img_latents)
...
def optimize_slf(latents, dirs, n_pers_pc):
    dirs = dirs.permute(0,3,1,2)
    local_batch_size = 256
    slf = SpherelatentField().cuda()
    sample_coords = torch.rand(n_pers_pc, local_batch_size, 1, 2).cuda() * 2. - 1 
    sample_coords = sample_coords.half()
    tot_step=10000
    init_lr_sp=1e-2
    optimizer_sp = torch.optim.Adam(slf.parameters(), lr=init_lr_sp)
    latents = rearrange(latents, 'b l c h w -> b (l c) h w')
    for step in range(tot_step):
        if step % 500 == 0:
            print(f'{step} / {tot_step}')
        sample_coords = torch.rand(n_pers_pc, local_batch_size, 1, 2).cuda() * 2. - 1
        # sample_coords = sample_coords.half()
        cur_latents_ = F.grid_sample(latents, sample_coords.half(), padding_mode='border') # latents: B C H W
        cur_dirs_ = F.grid_sample(dirs, sample_coords.float(), padding_mode='border') # dirs: B 3 H W
        cur_dirs = cur_dirs_[:, :, :, 0].permute(0, 2, 1)                                    # [n_pers, local_batch_size, 3]
        cur_dirs = cur_dirs / torch.linalg.norm(cur_dirs, 2, -1, True) # B N 3
        target_latents = cur_latents_[:, :, :, 0].permute(0, 2, 1) # B N C
        pred_latents = slf(cur_dirs.reshape(-1, 3), requires_grad=False)
        pred_latents = pred_latents.reshape(n_pers_pc, local_batch_size, -1).half()
        # pred_grads = pred_grads.reshape(n_pers, local_batch_size, 3)

        latent_loss = F.smooth_l1_loss(target_latents, pred_latents, beta=5e-1, reduction='mean')
    ...
    new_latents = slf(pano_dirs.reshape(-1, 3), requires_grad=False)
    new_latents = new_latents.detach().reshape(height, width, -1).half()
    new_latents = new_latents.permute(2, 0, 1)[None, ...]
    new_latents = rearrange(new_latents, 'b (l c) h w -> b l c h w', l=14, c=4)
    # new_latents = rearrange()
    return new_latents
...
class MaskStableVideoDiffusionPipeline(StableVideoDiffusionPipeline):
    ...
    def __call__(
        ...
        # sp_latent_field = SpherelatentField(fine_res = 192).cuda()
        pers_latents, pers_mask, pers_img_latents, pers_dirs, pers_n = split_latent(latents=latents, mask=mask, image_latents=image_latents,gen_res=32)
        
        # latents = pers_latents[pers_id][None,...]
        # image_latents = pers_img_latents[pers_id]
        # mask = pers_mask[pers_id]


        with torch.enable_grad():
            new_latents = optimize_slf(pers_latents, pers_dirs, pers_n)
        
        # latents = new_latents
        diff = torch.abs(new_latents - latents)
        d_min, d_max = diff.min(), diff.max()
        diff = (diff - d_min) / (d_max - d_min)
        print('max_err', d_max)
        for k in range(14):
            diff_k = diff[0, k, ...]
            diff_k = torch.mean(diff_k, 0)
            imageio.imwrite(f'diff_{k}.png', (diff_k * 255.0).detach().cpu().numpy().astype(np.uint8))
