...
def rj_encode_vae_image_2(self, image: torch.Tensor, device, num_videos_per_prompt, do_classifier_free_guidance):
    image = image.to(device=device)
    image_latents = self.vae.encode(image).latents#.mode()
...
class MaskStableVideoDiffusionPipeline(StableVideoDiffusionPipeline):
    ...
    def __call__(
        ...
        # 0. Default height and width to unet
        # self.vae = 
        # self.vae = AutoencoderTiny.from_pretrained("madebyollin/taesdxl", torch_dtype=torch.float16).cuda()
        ...
        # image_latents = self.rj_encode_vae_image_2(image, device, num_videos_per_prompt, do_classifier_free_guidance)
        # image_latents = rj_encode_vae_image_2(self, image, device, num_videos_per_prompt, do_classifier_free_guidance)
...
# latents, noise, self.pipe.scheduler.timesteps[t:t+1]
        # latents = self.scheduler.add_noise(latents, latents, self.scheduler.timesteps[10:13])
        # latents_np = np.load('latents.npy')
        # latents_xx = torch.from_numpy(latents_np).half().cuda()
        # frames = self.decode_latents(latents, num_frames, decode_chunk_size=1)
        ...
        if call_mode == 'decode_latents':
            ...
            if needs_upcasting:
                self.vae.to(dtype=torch.float16)
            # frames = decode_latents(self, latents, num_frames, decode_chunk_size)
            frames = self.decode_latents(latents, num_frames, decode_chunk_size=1)
            frames = svd_tensor2vid(frames, self.image_processor, output_type=output_type)
        ...
        # window_size = min(64, math.ceil(ll_width / 4))
        print(f'width={width}')
        if width == 4096:
            window_size = (128, 128)
            stride = (64, 64)
            ...
            assert width == 1536
            window_size=(64, 64)
            stride = (32, 32) # default circle
            # window_size=(32, 32)
            # stride = (16, 14) # 88-x / (32 - x)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for j, batch_view in enumerate(views_batch):
                    ...
                    # compute the previous noisy sample x_t -> x_t-1  TODO: CHECK HERE
        ...
        # count = None
        # value = None
        # latents_for_view = None
        # mask_for_view = None
        # latents_denoised_batch = None
        # noise_pred = None
        # image_latents = None
        # mask = None
        ...
        # if needs_upcasting:
        #     self.vae.to(dtype=torch.float16)
        # fuse_shape = image[:,:,None,:,:].repeat(1,1,num_frames,1,1).shape
        # fuse_img = torch.zeros(fuse_shape).half().cuda()
        # cnt_img = torch.zeros(fuse_shape).half().cuda()
        ...
        #     img_for_view = self.decode_latents(latents_for_view, num_frames, decode_chunk_size)
            
            
        #     # extract value from batch
        #     for img_for_view_t, (h_start, h_end, w_start, w_end) in zip(
        #         img_for_view.chunk(vb_size), batch_view
        #     ):
        #         sf__ = self.vae_scale_factor
        #         h_start, h_end, w_start, w_end = h_start * sf__, h_end * sf__, w_start * sf__, w_end * sf__
        #         if circular_padding and w_end > fuse_img.shape[4]:
        #             # Case for circular padding
        #             fuse_img[:, :, :, h_start:h_end, w_start:] += img_for_view_t[
        #                 :, :, :, :, : fuse_img.shape[4] - w_start
        #             ]
        #             fuse_img[:, :, :, h_start:h_end, : w_end - fuse_img.shape[4]] += img_for_view_t[
        #                 :, :, :, :, fuse_img.shape[4] - w_start :
        #             ]
        #             cnt_img[:, :, :, h_start:h_end, w_start:] += 1
        #             cnt_img[:, :, :, h_start:h_end, : w_end - fuse_img.shape[4]] += 1
        #         else:
        #             fuse_img[:, :, :, h_start:h_end, w_start:w_end] += img_for_view_t
        #             cnt_img[:, :, :, h_start:h_end, w_start:w_end] += 1

        # # take the MultiDiffusion step. Eq. 5 in MultiDiffusion paper: https://arxiv.org/abs/2302.08113
        # fuse_img = torch.where(cnt_img > 0, fuse_img / cnt_img, fuse_img)


        # frames = svd_tensor2vid(fuse_img, self.image_processor, output_type=output_type)
        # return StableVideoDiffusionPipelineOutput(frames=frames), mask
        # ---------------------------------------------------
