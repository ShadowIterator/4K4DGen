...
def panorama_to_pers_directions(gen_res=512, ratio=1., ex_rot=None):
    ...
    rot_inv_matrix = torch.from_numpy(rot_inv_matrix).cuda().float()
    ...
    all_dirs = _apply_rot(all_dirs, rot_inv_matrix)
    to_vecs = _apply_rot(to_vecs, rot_inv_matrix)
    down_vec = _apply_rot(down_vec, rot_inv_matrix)
    right_vecs = _apply_rot(right_vecs, rot_inv_matrix)
